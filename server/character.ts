...
function parseCharacter(
	...
	const sessionLabel =
		state.sessionActive && state.sessionCounter
			? `Session ${roman(state.sessionCounter)}`
			: null;
...
function parseAbilities(md: string): Character["abilities"] {
	...
	const rows =
		block.match(
			/^\|\s*(Strength|Dexterity|Constitution|Intelligence|Wisdom|Charisma).+$/gim,
		) ?? [];
	...
	for (const row of rows) {
		...
		const modifier =
			Number(modStr.replace(/^\+/, "")) || Math.floor((score - 10) / 2);
		result[key] = {
			score,
			modifier,
			save: modifier,
...
function parseSaves(
	md: string,
): Partial<Record<keyof Character["abilities"], boolean>> {
...
function parseSkills(md: string): Record<string, number> {
	...
	for (const line of block.split("\n")) {
		...
		const name = cells[1];
		const bonus = cells[3];
		if (!name || !bonus || /^Skill$/i.test(name) || /^---/.test(bonus))
			continue;
...
function parseSpellSlots(md: string): Character["spellSlots"] {
	...
	const re =
		/\*\*Spell slots\s*\((\d)(?:st|nd|rd|th)\):\*\*\s*(\d+)\s*\/\s*(\d+)/gi;
...
function parseFirstAttackBonus(
	md: string,
