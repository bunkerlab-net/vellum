...
An agent-driven, text-based D&D 5e (2014) tabletop RPG. The DM is an AI (Claude Code by default); the campaign is markdown
...
scope (all official 2014 sourcebooks), the campaign state layout, the dice protocol, and the live-persistence rules. Skills
...
Operators: `dlN` drop-lowest, `dhN` drop-highest, `khN` keep-highest, `klN` keep-lowest, `+N` / `-N` flat modifier. Chain
dice and constants with `+` or `-` for compound rolls (Guidance, Bless, Sneak Attack, Bane, Bardic Inspiration).
...
Your campaigns are personal user content, not part of the harness. `campaigns/` is `.gitignore`d, so characters, sessions,
and DM-only plot notes never get pushed. If you want backup or history of your campaigns, keep them in a separate private
repo or your usual backup setup.
...
The repo follows the `AGENTS.md` convention with `CLAUDE.md` as a symlink, applied both at the file level
(`AGENTS.md` / `CLAUDE.md`) and directory level (`.agents/` / `.claude`). Edit the `AGENTS.md` / `.agents/` side; the
symlinks just keep Claude Code's native conventions working.
