...
export default function App() {
				...
				case "assistant_partial": {
					...
					setEntries((es) => {
						if (partialEntryId.current != null) {
							return es.map((e) =>
								e.id === partialEntryId.current && e.type === "narrator"
									? { ...e, text: e.text + msg.text }
									: e,
							);
						}
						const id = nextId();
	...
	const { state, send, setPermissionMode, interrupt, restart } =
	...
	const submitAskAnswers = (
		...
		const text =
			lines.length === 1 ? (answers[0]?.join(", ") ?? "") : lines.join("\n");
		send(text);
		setThinking(true);
	};
	...
	const rollD20 = (label = "Charisma (Persuasion)", modifier?: number) => {
		const mod =
			...
			character?.abilities?.CHA?.modifier ??
			0;
	...
	const hasSelection =
		selection.campaign != null && selection.character != null;
	...
	const location = hasSelection
		? (character?.location ?? "Somewhere in the world")
		: "";
	const time = character?.inGameDate ?? "";
	const sessionLabel = character?.sessionLabel ?? null;
	const dayLabel = useMemo(
		() =>
			character?.sessionLabel && character?.campaign
				? character.campaign.replace(/-/g, " ")
				: null,
		[character],
	);
	...
	return (
		<div className="app">
			...
			<main className={`main-grid ${hasSelection ? "" : "single-col"}`}>
				...
				{hasSelection && (
					<CharacterPanel character={character} onRoll={() => rollD20()} />
				)}
			</main>
...
function parseAskQuestions(input: unknown): AskQuestion[] {
	...
	for (const q of raw) {
		...
		const options: { label: string; description?: string }[] = Array.isArray(
			obj.options,
		)
			? (obj.options as unknown[]).flatMap((o) => {
					...
					if (typeof oo.description === "string")
						opt.description = oo.description;
					return [opt];
				})
			: [];
		if (question && options.length > 0) {
			out.push({
				...
				multiSelect:
					typeof obj.multiSelect === "boolean" ? obj.multiSelect : false,
				options,
			});
		}
	}
