...
function StoryEntry({
			...
			<div className="story-block choice-block">
				<div className="choice-card">
					...
					<div className="choice-or">
						— or describe your own approach below —
					</div>
				</div>
			</div>
		);
	...
	// narrator
	const dropCap =
		revealMode === "illuminated" && entry.dropCap !== false && !entry.speaker;

	return (
		<div
			className={`story-block narrator-block ${revealMode}${dropCap ? " has-drop-cap" : ""}`}
		>
			{entry.speaker && (
			...
			<div
				className={`narrator-text ${revealed ? "revealed" : "hidden-reveal"}`}
			>
