...
- **Never reveal the ground truth of a failed check.** A failed Perception, Insight, Investigation, etc. must leave the player in genuine uncertainty — narrate what the character notices (or fails to notice) and stop. Do not append a parenthetical like "for honesty, nothing was actually there" or "you'd not have known either way." The point of the check is that the player doesn't know. Out-of-character reassurance destroys the tension and trains the player to dismiss failed checks. The DM is allowed — required, even — to keep secrets the dice didn't earn.
